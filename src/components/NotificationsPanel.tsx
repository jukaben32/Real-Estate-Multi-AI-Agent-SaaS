...
const TYPE_LABELS: Record<string, string> = {
  new_lead: 'New lead',
  appointment_booked: 'Viewing booked',
  appointment_cancelled: 'Viewing cancelled',
    ...
    <div>
      ...
      <div className="divide-y divide-[var(--border)]">
        {notifications.map((n) => (
          ...
          >
            <span
              className={`status-dot mt-1.5 shrink-0 ${n.is_read ? 'bg-gray-300' : 'bg-[var(--teal-500)]'}`}
            />
            <div className="min-w-0">
              <p className={`text-sm ${n.is_read ? 'text-[var(--text-3)]' : 'font-medium'}`}>{n.title}</p>
        ...
        {notifications.length === 0 && (
          <p className="py-6 text-center text-sm text-[var(--text-3)]">No notifications yet.</p>
        )}
      </div>
    </div>
  )
